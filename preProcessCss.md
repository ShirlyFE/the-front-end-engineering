...
sass安装：
    Sass是Ruby语言写的，所以必须先安装Ruby，然后再安装Sass。
...
Stylus的安装
    ...
    Stylus的安装和LESS在服务器端的安装很相似，同样依赖于Node JS的环境
命令行下的编译：
    ...
    stylus css //一个目录名为“css”将在同一个目录编译并输出“.css”文件
    stylus css –out public/stylesheets  //输出到“./public/stylesheets”
    stylus one.styl two.styl //同时转译多个文件


sass语法：
    sass文件后缀：.sass 或者 .scss
sass老的写法完全依靠严格的缩进来书写
Sass 中主要有六种数据类型：
...
变量只能作为属性值来用，例如 color: $color;，那变量能不能用在选择器或者属性上面吗？ yes
$name: foo;
$attr: border;
p.#{$name} {
  #{$attr}-color: blue;
...
Sass 运算注意事项：
a. 建议运算符之间空开一个空格。否则你会发现编译器把下面的 $h- 识别成了一个变量：
$w : 7px;
$h : 10px;
p:before {
  width: $h-$w;
...
/*sass的mixin需要使用"@mixin"来定义，使用@include来调用，也支持旧版的加好'+'调用；@mixin 定义的内容，不会编译输出。*/
/*声明一个Mixin叫作“error”*/
...
另一个支持使用map的原因，是它可以创建map-get()函数以提供友好API的功能
...
stylus使用遇到问题的时候直接查看其test的使用方式可以很快的解决大多数问题：
[stylus test](https://github.com/stylus/stylus/blob/master/test/run.js)
